...
|------|---------|---------|
| **ci.yml** | Push/PR to `master`, `staging` | CI: backend lint (mypy, ruff), backend tests in Docker (db + prestart + backend via `docker-compose.ci.yml`), frontend lint (Biome), frontend build. |
