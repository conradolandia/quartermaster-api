...
import { createUser, deleteUserAsSuperuser } from "./utils/privateApi"
...
test("User can reset password successfully using the link", async ({
  ...
  try {
    ...
    // Verify the new password is accepted: attempt login (user is non-superuser so gets 403, not 401)
    ...
    await expect(
      page.getByText("Only superusers can access the dashboard"),
    ).toBeVisible({ timeout: 5000 })
  } finally {
    await deleteUserAsSuperuser(request, String(user.id))
  }
})
